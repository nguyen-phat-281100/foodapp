...
public class PlaceYourOrderActivity extends AppCompatActivity {
    ...
    private void calculateTotalAmount(RestaurantModel restaurantModel) {
        int subTotalAmount = (int) 0f;

        for(Menu m : restaurantModel.getMenus()) {
            subTotalAmount += m.getPrice() * m.getTotalInCart();
        }

        tvSubtotalAmount.setText(String.format("%d", subTotalAmount)+" VND");
        if(isDeliveryOn) {
            tvDeliveryChargeAmount.setText(String.format("d", restaurantModel.getDelivery_charge())+" VND");
            subTotalAmount += restaurantModel.getDelivery_charge();
        }
        tvTotalAmount.setText(String.format("%d", subTotalAmount)+" VND");
    }
